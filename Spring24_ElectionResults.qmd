...
```{r}
second.year.results <- rcv.results(position = "second_senator")
...
third.year.results <- rcv.results(position = "third_senator")
...
international.results <- rcv.results(position = "international_senator", seats = 2)
...
-   Domanic Dick
    ...
    ordered <- c(
      candidates, 
      winners[which(!fourth.year.candidates %in% fourth.year.winners)]
    )
...
turnout.df <- read_csv("sp24_turnout_results.csv", col_select = c("Race", "Votes", "Eligible")) |> 
  head(7) |>
...
read_csv("asa_historical_turnout.csv") |>
  head(8) |>
