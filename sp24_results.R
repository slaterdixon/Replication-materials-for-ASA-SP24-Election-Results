### Tabulate RCV results from a CampusLabs Election using vote::rcv()
### Author: Slater Dixon (slaterjdixon@gmail.com)
### Description: This script tabulates election results for a CampusLabs Election 
###               conducted using RCV. It ingests a raw file from CampusLabs

### "ElectionResults.csv": Raw election results downloaded from CampusLabs
### (Viking Central at Augustana University)
### -> Election results for each race (csv)
### -> Bar plot showing each round for each race (png)



all.results <- read_csv("SP_24_ElectionResults.csv", skip = 1) |>
...
pivoted.results <- function(every.result, position = "large_senator", seats = 4){
  ...
  # -> pivoted.result (tbl): wide df of form Vote x Cand. and no id col
  ...
  every.result |>
    ...
    # Pivot to form required by `vote::rcv()` and drop id col
...
plot.race <- function(every.result, position, seats = 4){
  ...
  # Get pivoted results for the given position
  calculate.votes <- pivoted.results(every.result, position)

  # Get cutoffs for each round
  cutoffs <- tibble(cutoff = calculate.votes$quotas) |>
    rowid_to_column(var = "stage")
  
  # Create table of results and join to cutoffs 
  stages <- calculate.votes$preferences |> 
    as_tibble() |>
    rowid_to_column(var = "stage") |>
    pivot_longer(-stage) |>
    left_join(cutoffs) |>
  ...
  no.candidates <- calculate.votes |> colnames() |> length() |> print()
  ...
  incumbents <- read_csv("senators23-24.csv") |> pull(Name)
  ...
  mutate(name.label = str_replace_all(name.label, pattern = "[-]", "-** \n\n**")) 
  ...
  write_csv(summary(calculate.votes), paste0("./out/reports/", position, "_IRV_results.csv"))
  print(paste0("Wrote ./out/reports/", position, "_IRV_results.csv"))
  
  ggsave(filename = paste0(position, "_IRV_results.jpg"), 
         plot = stages.plot, 
         path = "./out/plots/")
...
map(
  positions[!positions %in% c("fourth_senator", "international_senator")] ,
 ~ process.race(results.pivoted, position = .)
)

process.race(results.pivoted, position = "international_senator", seats = 2)


df <- read_csv("./out/reports/international_senator_IRV_results.csv")
